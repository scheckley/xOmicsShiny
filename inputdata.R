...
observe({
  ...
  if (input$select_dataset == "Saved Projects in Database") {
    library(DBI)
    library(pool)
    pool <- dbPool(RMariaDB::MariaDB(),
      dbname = "rshiny", host = MariaDB_configure$host, port = MariaDB_configure$port,
  ...
  if (input$select_dataset == "Public Data(DiseaseLand)") {
    ...
    library(pool)
    pool <- dbPool(RMariaDB::MariaDB(),
      dbname = "diseaseland", host = MariaDB_configure$host, port = MariaDB_configure$port,
      user = MariaDB_configure$user, password = MariaDB_configure$password
    )
...
names(returnlist) <- c(
  "ProjectID", "Name", "Species", "ShortName", "Path", "file1", "file2",
...
DataReactiveRData <- reactive({
  withProgress(message = "Fetching data.", detail = "This may take a while...", value = 0, {
    ...
    if (exists("comp_info")) {
      sel_comp <- data.frame(Comparison = rownames(comp_info), comp_info) %>% dplyr::filter(Group_name != "", !is.na(Group_name))
      dim(sel_comp)
    ...
    if (exists("results_stat")) {
      ...
      pcutoffdf <- rbind(
        data.frame("Type" = "Pvalue", "LessThan0.05" = sum(results_stat$pvalue < 0.05, na.rm = TRUE), "LessThan0.01" = sum(results_stat$pvalue < 0.01, na.rm = TRUE), "LessThan0.001" = sum(results_stat$pvalue < 0.001, na.rm = TRUE)),
    ...
    if (exists("results_omics")) {
      results_omics <- results_omics %>%
        dplyr::filter(model != "NA") %>%
        dplyr::rename(any_of(lookup2))
...
DataReactiveDB <- reactive({
  withProgress(message = "Fetching data.", detail = "This may take a while...", value = 0, {
    ...
    ProteinGeneName <- geneannotation %>%
      dplyr::select(one_of(c("UniqueID", "GeneName", "GeneID"))) %>%
...
output$upload.message <- renderText({
  upload_message()
})

# *** new csv code ***

# Replace the existing observeEvent(input$uploadData, {...}) in inputdata.R with this:
...
observeEvent(input$uploadData, {
  withProgress(message = "Process uploaded files.", detail = "This may take a while...", value = 0, {
    ...
    req(input$F_project_name != "" &
      ((!is.null(input$F_sample) & !is.null(input$F_exp)) | (!is.null(input$F_comp))))

    tryCatch(
      {
        # Use the new CSV conversion function with biomaRt support
        returnlist <- convert_csv_to_rdata(
          sample_file_path = input$F_sample$datapath,
          exp_file_path = input$F_exp$datapath,
          comp_file_path = if (!is.null(input$F_comp)) input$F_comp$datapath else NULL,
          protein_id_file_path = if (!is.null(input$F_annot) && input$F_annot_auto == 0) input$F_annot$datapath else NULL,
          project_name = input$F_project_name,
          species = input$Fspecies,
          auto_annotate = (input$F_annot_auto == 1),
          id_type = if (input$F_annot_auto == 1) input$F_ID_type else NULL,
          include_description = (input$F_description == 1),
          fill_missing_names = (input$F_fillName == 1),
          create_network = FALSE, # Set to TRUE if you want network analysis
          temp_dir = "unlisted/"
        )

        cat("Finished processing data files for", returnlist$ProjectID, ".\n")
        up_message2 <- str_c("The direct URL for the uploaded dataset is: ", URL, "/?unlisted=", returnlist$ProjectID)
        upload_message(up_message2)

        # Store the result for use by DataReactiveTxt
        DataReactiveTxt(returnlist)
      },
      error = function(e) {
        error_message <- paste("Error processing CSV files:", e$message)
        upload_message(error_message)
        shinyalert("Error!", error_message, type = "error")
      }
    )
  })
})
...
observeEvent(input$load | input$adddata | input$uploadData | input$customData, {
  ...
  } else if (!is.null(input$load)) {
    DataIn <- DataReactiveRData()
  } else if (!is.null(input$adddata)) {
    DataIn <- DataReactiveRData()
...
group_info <- reactive({
  req(length(working_project()) > 0)
  req(DataInSets[[working_project()]]$MetaData)
  group_info <- DataInSets[[working_project()]]$MetaData %>%
...
output$comp_info <- renderUI({
  ...
  output$comparison <- DT::renderDT(server = FALSE, {
    DT::datatable(DataInSets[[working_project()]]$comp_info,
      extensions = "Buttons",
      options = list(
        dom = "lBfrtip", pageLength = 15,
        buttons = list(
...
output$ProteinGeneName <- DT::renderDataTable(server = TRUE, {
  req(length(working_project()) > 0)
  req(DataInSets[[working_project()]]$ProteinGeneName)
  DT::datatable(DataInSets[[working_project()]]$ProteinGeneName,
    extensions = "Buttons",
    options = list(
      dom = "lBfrtip", pageLength = 15,
      buttons = list(
